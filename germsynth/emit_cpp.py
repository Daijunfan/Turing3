#!/usr/bin/env python3
"""Emit a standalone C++17 verifier/implementation from a germ certificate."""
...
def emit(certificate: Path, output: Path) -> None:
    ...
    if data["problem"]["kind"] != "matrix_multiplication":
        raise ValueError("this generator currently emits matrix multiplication only")
    terms = data["terms"]
    rank = len(terms)
    u = ",\n    ".join(c_array(term["u"]) for term in terms)
    v = ",\n    ".join(c_array(term["v"]) for term in terms)
    w = ",\n    ".join(c_array(term["w"]) for term in terms)
    source = f'''// Generated from {certificate.name}; do not edit coefficient arrays by hand.
#include <array>
#include <cassert>
#include <cstdint>
#include <iostream>
